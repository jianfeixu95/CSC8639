...
def preprocess_pneumonia_data(test_fac=0.15):
    ...
    # create labels
    df["label"] = df["bbox"].apply(lambda x: 0 if x == None else 1)
    df["class"] = df["bbox"].apply(lambda x: "Normal" if x == None else "Pneumonia")

    # no encoded pixels mean healthy
    df["image"] = df["patientId"].apply(lambda x: PNEUMONIA_IMG_DIR / (x + ".jpg"))
