...
path_chest_imagenome = "Data/chest-imagenome"

txt_file_to_log_stats = "Model/report-generation-model/code/src/dataset/dataset_stats_mini.txt"
...
def compute_and_print_stats_for_csv_files(csv_files_dict, image_ids_to_avoid):
    total_stats = {
        ...
        for stat in [
            ...
            # bboxes that have bbox names (like 'left breast' etc.) that are not in the 29 anatomical regions are considered outliers
        ]
    }
    total_stats["total_bbox_with_phrases_counter_dict"] = defaultdict(
        int)  # dict to count how often each of the 29 anatomical regions have phrases
    total_stats["total_outlier_bbox_counter_dict"] = defaultdict(
        int)  # dict to count how often each of the outlier regions have phrases
