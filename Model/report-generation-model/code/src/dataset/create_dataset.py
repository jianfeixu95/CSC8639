...
class CreateDataset(object):
    def __init__(
        ...
        self.txt_file_for_logging = txt_file_for_logging
        self.num_rows_to_create_in_new_csv_files = None if num_rows_to_create_in_new_csv_files == "None" else num_rows_to_create_in_new_csv_files
    ...
    def write_stats_to_log_file(
        ...
        with open(self.txt_file_for_logging, "a") as f:
            f.write(f"{dataset}:\n")
    ...
    def write_rows_in_new_csv_file(self, dataset: str, csv_rows: list[list]) -> None:
        ...
        self.log.info(f"Writing rows into new {dataset}.csv file...")
        if dataset == "test":
            csv_rows, csv_rows_less_than_29_regions = csv_rows

        new_csv_file_path = os.path.join(self.path_full_dataset, dataset)
        new_csv_file_path += ".csv"
    ...
    def get_reference_report(self, subject_id: str, study_id: str, missing_reports: list[str]):
        ...
        path_to_report = os.path.join(self.path_mimic_cxr, "files", f"p{subject_id[:2]}", f"p{subject_id}", f"s{study_id}.txt")
    ...
    def get_rows(self, dataset: str, path_csv_file: str, image_ids_to_avoid: set) -> list[list]:
        ...
        with open(path_csv_file) as csv_file:
            ...
            for row in tqdm(csv_reader, total=total_num_rows):
                ...
                if self.num_rows_to_create_in_new_csv_files and num_rows_created >= self.num_rows_to_create_in_new_csv_files:
                    break
    ...
    def create_new_csv_file(self, dataset: str, path_csv_file: str, image_ids_to_avoid: set) -> None:
        self.log.info(f"Creating new {dataset}.csv file...")
        ...
        csv_rows = self.get_rows(dataset, path_csv_file, image_ids_to_avoid)

        # write those rows into a new csv file
        self.write_rows_in_new_csv_file(dataset, csv_rows)

        self.log.info(f"Creating new {dataset}.csv file... DONE!")


    def create_new_csv_files(self, csv_files_dict, image_ids_to_avoid):
        if os.path.exists(self.path_full_dataset):
            if self.auto_delete_dataset_folder:
                self.log.info(f"Full dataset folder has already been automatically deleted at {self.path_full_dataset}.")
                shutil.rmtree(self.path_full_dataset)
            else:
                self.log.error(f"Full dataset folder already exists at {self.path_full_dataset}.")
                self.log.error("Delete dataset folder or rename variable path_full_dataset in src/path_datasets_and_weights.py before running script to create new folder!")
                return None

        os.mkdir(self.path_full_dataset)
        for dataset, path_csv_file in csv_files_dict.items():
            self.create_new_csv_file(dataset, path_csv_file, image_ids_to_avoid)

    def get_images_to_avoid(self):
        path_to_images_to_avoid = os.path.join(self.path_chest_imagenome, "silver_dataset", "splits", "images_to_avoid.csv")
    ...
    def create(self):
        csv_files_dict = self.get_train_val_test_csv_files()
        image_ids_to_avoid = self.get_images_to_avoid()
        self.create_new_csv_files(csv_files_dict, image_ids_to_avoid)