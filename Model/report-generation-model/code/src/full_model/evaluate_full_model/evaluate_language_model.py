...
def compute_clinical_efficacy_scores(
        language_model_scores: dict, gen_reports: list[str], ref_reports: list[str], path_chexbert_weights):
    """
    ...
    def get_chexbert(path_chexbert_weights):
        model = bert_labeler()
    ...
    chexbert = get_chexbert(path_chexbert_weights)
...
def compute_language_model_scores(gen_and_ref_sentences, gen_and_ref_reports, path_chexbert_weights):
    ...
    def compute_report_level_scores():
        ...
        compute_clinical_efficacy_scores(
            language_model_scores, gen_reports, ref_reports, path_chexbert_weights)
...
def write_sentences_and_reports_to_file(
    ...
    def write_sentences():
        ...
        txt_file_name_abnormal = os.path.join(generated_sentences_and_reports_folder_path, "generated_sentences", f"generated_abnormal_sentences_step_{overall_steps_taken}.txt")
...
def evaluate_language_model(
        model, val_dl, tokenizer, writer, run_params,
        generated_sentences_and_reports_folder_path,
        batch_size,
        num_beams,
    ...
    language_model_scores = compute_language_model_scores(gen_and_ref_sentences, gen_and_ref_reports, path_chexbert_weights)
