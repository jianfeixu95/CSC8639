...
# RUN = 2
# CHECKPOINT = "checkpoint_val_loss_64.803_overall_steps_502.pt"
...
class EvaluateFullModel(object):

    def __init__(
            self,
            run,
            seed,
            name_of_best_full_model_checkpoint,
        ...
        self.run = run
        self.seed = seed
        self.name_of_best_full_model_checkpoint = name_of_best_full_model_checkpoint
        ...
        self.path_test_set_evaluation_scores_txt_files = path_test_set_evaluation_scores_txt_files
        self.path_to_best_object_detector_weights = path_to_best_object_detector_weights

        # dataset
        self.auto_delete_dataset_folder = auto_delete_dataset_folder
        ...
        self.final_scores_txt_file = os.path.join(self.path_test_set_evaluation_scores_txt_files, "final_scores.txt")
    ...
    def write_all_scores_to_file(
        ...
        def write_obj_detector_scores():
            with open(self.final_scores_txt_file, "a") as f:
                f.write(
        ...
        with open(self.final_scores_txt_file, "a") as f:
            ...
            f.write(f"Checkpoint: {self.name_of_best_full_model_checkpoint}\n")
            f.write(f"BertScore for removing similar generated sentences: {self.bertscore_similarity_threshold}\n")
            f.write(f"Num beams: {self.num_beams}\n")
    ...
    def write_sentences_and_reports_to_file_for_test_set(
        ...
        def write_sentences():
            txt_file_name = os.path.join(self.path_test_set_evaluation_scores_txt_files, "generated_sentences.txt")
            txt_file_name_abnormal = os.path.join(self.path_test_set_evaluation_scores_txt_files,
                                                  "generated_abnormal_sentences.txt")
            ...
            with open(txt_file_name_abnormal, "a") as f:
                for gen_sent, ref_sent in zip(generated_sentences_abnormal_regions,
                                              reference_sentences_abnormal_regions):
                    f.write(f"Generated sentence: {gen_sent}\n")
                    f.write(f"Reference sentence: {ref_sent}\n\n")

        def write_reports():
            txt_file_name = os.path.join(self.path_test_set_evaluation_scores_txt_files, "generated_reports.txt")
        ...
        num_generated_sentences_to_save = self.num_batches_of_generated_sentences_to_save_to_file * self.batch_size
        num_generated_reports_to_save = self.num_batches_of_generated_reports_to_save_to_file * self.batch_size

        # all below are list of str
        generated_sentences = gen_and_ref_sentences["generated_sentences"][:num_generated_sentences_to_save]
        generated_sentences_abnormal_regions = gen_and_ref_sentences["generated_sentences_abnormal_selected_regions"][
        ...
        reference_sentences_abnormal_regions = gen_and_ref_sentences["reference_sentences_abnormal_selected_regions"][
                                               :num_generated_sentences_to_save]
    ...
    def evaluate_language_model_on_test_set(
            self, model, test_loader, test_2_loader, tokenizer):
        def iterate_over_test_loader(test_loader):
            ...
            with torch.no_grad():
                for num_batch, batch in tqdm(enumerate(test_loader)):
                    ...
                    ) = get_sents_for_normal_abnormal_selected_regions(region_is_abnormal, selected_regions,
                    ...
                    gen_and_ref_sentences["generated_sentences_normal_selected_regions"].extend(
                        gen_sents_for_normal_selected_regions)
                    gen_and_ref_sentences["generated_sentences_abnormal_selected_regions"].extend(
                        gen_sents_for_abnormal_selected_regions)
                    gen_and_ref_sentences["reference_sentences"].extend(reference_sents_for_selected_regions)
                    gen_and_ref_sentences["reference_sentences_normal_selected_regions"].extend(
                        ref_sents_for_normal_selected_regions)
                    gen_and_ref_sentences["reference_sentences_abnormal_selected_regions"].extend(
                        ref_sents_for_abnormal_selected_regions)
                    ...
                    update_gen_and_ref_sentences_for_regions(gen_and_ref_sentences,
                                                             generated_sents_for_selected_regions,
                                                             reference_sents_for_selected_regions, selected_regions)
                    update_num_generated_sentences_per_image(gen_and_ref_sentences, selected_regions)

                    if num_batch < self.num_batches_of_generated_reports_to_save_to_file:
                        update_gen_sentences_with_corresponding_regions(gen_sentences_with_corresponding_regions,
                                                                        generated_sents_for_selected_regions,
                                                                        selected_regions)
        ...
        self.log.info("Test loader: generating sentences/reports...")
        iterate_over_test_loader(test_loader)
        self.log.info("Test loader: generating sentences/reports... DONE.")

        self.log.info("Test loader 2: generating sentences/reports...")
        iterate_over_test_loader(test_2_loader)
        self.log.info("Test loader 2: generating sentences/reports... DONE.")

        self.write_sentences_and_reports_to_file_for_test_set(
            gen_and_ref_sentences,
            gen_and_ref_reports,
            gen_sentences_with_corresponding_regions,
        )
    ...
    def update_object_detector_metrics_test_loader_2(
        ...
        def get_gt_boxes_missing(targets):
            ...
            gt_boxes_missing = torch.tensor(gt_boxes_missing, device=self.device)
                ...
                for gt_boxes_missing_bool in gt_boxes_missing_single_image:
                    if gt_boxes_missing_bool:
                        gt_boxes_single_image.append(
                            torch.tensor([0, 0, 0, 0], dtype=boxes_single_image.dtype, device=self.device))
                    else:
    ...
    def evaluate_obj_detector_and_binary_classifiers_on_test_set(
            self, model, test_loader, test_2_loader):
        def iterate_over_test_loader(test_loader, num_images, is_test_2_loader):
            ...
            with torch.no_grad():
                for num_batch, batch in tqdm(enumerate(test_loader)):
                    ...
                    image_targets = [{k: v.to(self.device, non_blocking=True) for k, v in t.items()} for t in
                                     image_targets]
                    ...
                    try:
                        with torch.autocast(device_type="cuda", dtype=torch.float16):
                            output = model(
                                images, image_targets, input_ids, attention_mask, region_has_sentence,
                                region_is_abnormal
                            )
                    except RuntimeError as e:  # out of memory error
                        if "out of memory" in str(e):
                    ...
                    if is_test_2_loader:
                        self.update_object_detector_metrics_test_loader_2(obj_detector_scores, detections,
                                                                          image_targets,
                                                                          class_detected)
                    else:
        ...
        """
        obj_detector_scores = {}
        obj_detector_scores["sum_intersection_area_per_region"] = torch.zeros(29, device=self.device)
        obj_detector_scores["sum_union_area_per_region"] = torch.zeros(29, device=self.device)
        obj_detector_scores["sum_region_detected"] = torch.zeros(29, device=self.device)
        ...
        for subset in ["all", "normal", "abnormal"]:
            region_selection_scores[subset] = {
                "precision": torchmetrics.Precision(num_classes=2, average=None).to(self.device),
                "recall": torchmetrics.Recall(num_classes=2, average=None).to(self.device),
                "f1": torchmetrics.F1Score(num_classes=2, average=None).to(self.device),
            }
        ...
        for subset in region_selection_scores:
            for metric, score in region_selection_scores[subset].items():
                region_selection_scores[subset][metric] = score.compute()[
                    1].item()  # only report results for the positive class (hence [1])
    ...
    def evaluate_model_on_test_set(self, model, test_loader, test_2_loader, tokenizer):
        obj_detector_scores, region_selection_scores, region_abnormal_scores = self.evaluate_obj_detector_and_binary_classifiers_on_test_set(
            model, test_loader, test_2_loader)

        language_model_scores = self.evaluate_language_model_on_test_set(model, test_loader, test_2_loader, tokenizer)

        self.write_all_scores_to_file(
            obj_detector_scores,
            region_selection_scores,
            region_abnormal_scores,
            language_model_scores
    ...
    def get_model(self):
        checkpoint = torch.load(
            os.path.join(self.path_runs_full_model, f"run_{self.run}", "checkpoints",
                         f"{self.name_of_best_full_model_checkpoint}"),
    ...
    def get_data_loaders(self, tokenizer, test_dataset_complete, test_2_dataset_complete):
    ...
    def get_tokenized_dataset(self, tokenizer, raw_test_dataset, raw_test_2_dataset):
    ...
    def get_dataset(self):
        ...
        datasets_as_dfs = {}
        datasets_as_dfs["test"] = pd.read_csv(os.path.join(self.path_full_dataset, "test.csv"), usecols=usecols,
                                              converters=converters)
        datasets_as_dfs["test-2"] = pd.read_csv(os.path.join(self.path_full_dataset, "test-2.csv"), usecols=usecols,
                                                converters=converters)
    ...
    def evaluate(self):
        raw_test_dataset, raw_test_2_dataset = self.get_dataset()
        ...
        tokenized_test_dataset, tokenized_test_2_dataset = self.get_tokenized_dataset(tokenizer, raw_test_dataset,
                                                                                      raw_test_2_dataset)

        test_transforms = self.get_transforms()

        test_dataset_complete = CustomDataset("test", tokenized_test_dataset, test_transforms, self.log)
        test_2_dataset_complete = CustomDataset("test", tokenized_test_2_dataset, test_transforms, self.log)

        test_loader, test_2_loader = self.get_data_loaders(tokenizer, test_dataset_complete, test_2_dataset_complete)

        model = self.get_model()

        self.evaluate_model_on_test_set(model, test_loader, test_2_loader, tokenizer)
